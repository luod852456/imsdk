...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) ImSdkLibrary.getInstance().getContext().getSystemService(ImSdkLibrary.getInstance().getContext().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(ImSdkLibrary.getInstance().getContext());
        Intent notificationIntent = new Intent(ImSdkLibrary.getInstance().getContext(), ImSdkLibrary.getInstance().getMainActivityClass());
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(ImSdkLibrary.getInstance().getContext(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager)ImSdkLibrary.getInstance().getContext().getSystemService(ImSdkLibrary.getInstance().getContext().NOTIFICATION_SERVICE);
